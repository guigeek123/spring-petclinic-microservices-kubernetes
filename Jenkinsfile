...
def appName = 'discovery-server'
def appPort = 8761
def artifactName = 'discovery-server-2.0.6.BUILD-SNAPSHOT.jar'
    ...
    stages {
        ...
        stage('Deploy to Acceptance') {
            ...
            steps {
                container('kubectl') {
                    //Deploy application
                    sh("kubectl --namespace=acceptance apply -f k8s/app/")
                }
            }
        }
        ...
        stage('Deploy to Production') {
            ...
            steps {
                container('kubectl') {
                    //Deploy application
                    sh("kubectl --namespace=production apply -f k8s/app/")
                    //Display access
